...
const Likes = ({ post }) => {
  ...
  const [likesNumber, updateLikes] = useState(0);
  const [currentLikeObject, setLikeObject] = useState({});
  ...
  useEffect(() => {
    ...
    updateLikes(post.likes.data.length);

    for (const prop in post.likes.data) {
      if (post.likes.data[prop].userName === user){

        setLikeObject(post.likes.data[prop]);
        setLiked(true);
 ...
 const getLikeId = () => {
  for (const prop in post.likes.data) {
    ...
    console.log(prop);
    if (post.likes.data[prop].userName === user){
      console.log("hello " + user + " we are going to dislike this post");
      console.log(post.likes.data[prop]._id);
      return post.likes.data[prop]._id;
